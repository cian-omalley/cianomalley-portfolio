...
**3D:** quiet chamber, vertical light column rendering the timeline as ascending nodes. **Page:** portrait/intro (owner-supplied only), what-I-build summary, skills/technologies (from taxonomies), timeline (from `timeline_entry`, type-icons, date-sorted), CV download (hosted on the primary site, `cianomalley.works`), links to featured work. **No invented employers/qualifications — sections render only when content exists.**
